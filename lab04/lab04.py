...
class ArrayList:
    ...
    def append(self, value):
        ...
        if len(self.data) <= self.len:
            newarr = ConstrainedList(2 * len(self.data))
    ...
    def insert(self, idx, value):
        ...
        if ndx > self.len:
            ...
            self.append(None)
            for i in range(self.len-1, ndx, -1):
                self.data[i] = self.data[i-1]
            self.data[ndx] = value
        else:
            for i in range(self.len, ndx, -1):
                self.data[i] = self.data[i-1]
            self.data[ndx] = value
            self.len += 1
        ### END SOLUTION
    ...
    def index(self, value, i=0, j=None):
        ...
        ### BEGIN SOLUTION
        if j == None:
            for k in range(self._normalize_idx(i), self.len):
                if self[k] == value:
                    return k
            raise ValueError
        else:
