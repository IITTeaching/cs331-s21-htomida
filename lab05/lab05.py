...
class LinkedList:
    ...
    def cursor_insert(self, value):
        ...
        new node"""
        ### BEGIN SOLUTION
        n = self.Node(value, self.cursor, self.cursor.next)
    ...
    def insert(self, idx, value):
        ...
        ### BEGIN SOLUTION
        n = self.get_node(idx)
        n_insert = self.Node(value, n.prior, n)
        n.prior.next = n_insert
        n.prior = n_insert
        self.length += 1
        ### END SOLUTION
    ...
    def index(self, value, i=0, j=None):
        ...
        ### BEGIN SOLUTION
        if j == None:
            for k in range(self._normalize_idx(i), len(self)):
                if self.get_node(k).val == value:
                    return k
            raise ValueError
        else:
            for k in range(self._normalize_idx(i), self._normalize_idx(j)):
                if self.get_node(k).val == value:
                    return k
            raise ValueError
        ### END SOLUTION
